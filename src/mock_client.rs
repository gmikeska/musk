...
impl NodeClient for MockClient {
    fn send_to_address(&self, addr: &Address, amount: u64) -> ClientResult<Txid> {
        ...
        let txid = Txid::from_raw_hash(
            elements::hashes::sha256d::Hash::from_byte_array(
                rand::random::<[u8; 32]>()
            )
        );
        ...
        let tx = Transaction {
            ...
            output: vec![TxOut {
                ...
                asset: confidential::Asset::Explicit(AssetId::from_slice(&[0u8; 32]).expect("valid asset")),
    ...
    fn get_transaction(&self, txid: &Txid) -> ClientResult<Transaction> {
        ...
        inner.transactions.get(txid).cloned().ok_or_else(|| {
            ContractError::IoError(std::io::Error::other("Transaction not found"))
        })
    }
    ...
    fn get_new_address(&self) -> ClientResult<Address> {
        ...
        let secret_key = secp256k1::SecretKey::from_slice(&secret_bytes)
            .map_err(|e| ContractError::IoError(std::io::Error::other(format!("Key error: {e}"))))?;
        let secp_pubkey = secp256k1::PublicKey::from_secret_key(&secp, &secret_key);
        let bitcoin_pubkey = PublicKey::new(secp_pubkey);
        
        Ok(Address::p2wpkh(&bitcoin_pubkey, None, &AddressParams::ELEMENTS))
    }
}
