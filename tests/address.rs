...
fn test_taproot_address_format() {
    ...
    assert!(regtest_addr.to_string().starts_with("ert1p"), "Regtest taproot address should start with ert1p");
...
fn test_control_block_exists() {
    ...
    assert!(control_block.is_some(), "Control block should exist for the contract script");
}
...
fn test_different_contracts_different_addresses() {
    ...
    let contract2 = Contract::from_source("fn main() { let x: u32 = 1; assert!(jet::eq_32(x, 1)); }").unwrap();
    let compiled2 = contract2.instantiate(Arguments::default()).unwrap();
    ...
    assert_ne!(addr1, addr2, "Different contracts should have different addresses");
}
